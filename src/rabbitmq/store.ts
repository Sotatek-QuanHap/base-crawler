...
export class Store {
  ...
  public static instanceId: string = `crawler-instance-${Math.round(Math.random() * Date.now())}`;
  ...
  static async pingAPIEtherScans(chain: string): Promise<void> {
    const grpcUrl = this.config.get(`CONFIG_API_PING_ETHER_SCAN_URL_${chain}`.toUpperCase(), `${this.config.get('BASE_URL', 'http://localhost:4156')}/api/v1/configs/ping/api_ether_scan_of_${chain}/${this.instanceId}`);
  ...
  static async loadAPIEtherScans(chain: string, isReplace: boolean = false): Promise<void> {
    if (!isReplace && this.grpcs[chain])
      return;
    const grpcUrl = this.config.get(`CONFIG_API_ETHER_SCAN_URL_${chain}`.toUpperCase(), `${this.config.get('BASE_URL', 'http://localhost:4156')}/api/v1/configs/api_ether_scan_of_${chain}`);
    ...
    this.apiEtherScans[chain] = (res.data?.data?.stringValue || '').split(',').map((s: string) => s.trim());
  }

  static async loadGrpcs(chain: string, isReplace: boolean = false): Promise<void> {
    if (!isReplace && this.grpcs[chain])
      return;
    const grpcUrl = this.config.get(`CONFIG_GRPC_URL_${chain}`.toUpperCase(), `${this.config.get('BASE_URL', 'http://localhost:4156')}/api/v1/configs/grpc_of_${chain}`);
  ...
  static async loadCoinMaps(chain: string, isReplace: boolean = false): Promise<void> {
    ...
    const url = this.config.get(`CONFIG_ADDRESS_LISTEN_URL_OF_${chain}`.toUpperCase(), `${this.config.get('BASE_URL', 'http://localhost:4156')}/api/v1/configs/address_listen_of_${chain}`);
